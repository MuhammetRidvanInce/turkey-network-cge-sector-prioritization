...
This section first classifies sectors descriptively using the level values of the main network and general-equilibrium indices. Because the two indices are constructed independently and capture distinct information, sectors of strategic importance for the Turkish economy should demonstrate strength on both dimensions rather than excel on only one. The same two axes are then evaluated more selectively using percentile ranks, Pareto dominance, balance, and specification robustness. The initial mean-threshold screening is therefore used as the starting point for, but not conflated with, final scientific core selection.
...
Figure 8 places all 62 sectors in a two-dimensional percentile-rank plane, with the size-controlled EIPI percentile ($E_i$) on the horizontal axis and the $SNII\_DIM$ percentile ($S_i$) on the vertical axis. Pareto dominance refines the preliminary screening in Figure 7 by ranking its nine **Strategic Core** candidates according to their relative strength on both dimensions. Of these candidates, `sec29` and `sec44` reach Pareto-1, while the others are assigned to lower layers because stronger two-dimensional profiles dominate them. Because the Pareto analysis covers all 62 sectors, it also identifies `sec48` as a Pareto-1 sector even though it is not a Figure 7 candidate: its first-place network rank ($S=1.000$) prevents any other sector from dominating it on both axes.

Pareto membership is complemented by the hybrid balance score. `sec29` ($H=0.935$) and `sec44` ($H=0.852$) combine frontier status with strong balance, whereas `sec48` ($H=0.617$) has a more network-heavy profile. Likewise, `sec30` is highly balanced ($H=0.816$) but remains in Layer 2 because it is dominated by `sec29`. The balance score therefore helps distinguish well-rounded candidates within the Pareto ordering without replacing the dominance rule. The next subsection tests this refined selection for specification robustness.

### 4.2. Specification Robustness

The sensitivity of the three main-model candidates to measurement and aggregation methods is tested across 96 specifications. These are the full factorial combination of four SNII methods (`DIM`, `TOPSIS`, `PCA`, and `AVG`), four size-controlled EIPI variants, two normalizations/transformations (percentile and min–max), and three balance rules: geometric balance, which limits compensation between the two axes; the bottleneck rule, which evaluates a sector by its weaker axis; and their equally weighted hybrid.
...
Table 23 presents four explicit policy scenarios and the coefficients assigned to network topology ($w_N$) and the economic channels ($w_1, \dots, w_4$). These weights are not estimated preference parameters or claims about the actual priorities of Turkish policymakers; they are transparent, illustrative configurations designed to compare the consequences of emphasizing different policy objectives. The balanced scenario provides an equal-weight benchmark, while each targeted scenario assigns its largest weight to the defining objective and distributes the remaining weight across the other dimensions. Because individual scenario weights are necessarily normative, the analysis does not rely on these four configurations alone: 10,000 random draws from a symmetric Dirichlet$(1,1,1,1,1)$ distribution are used to evaluate ranking robustness across the full preference simplex.
...
### 5.1. Integrated Assessment of H1–H4 Findings
